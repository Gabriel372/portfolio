...
import ImgFalaTuDesktop from "../images/FalaTuDesktop.png";
import ImgFalaTuMobile from "../images/FalaTuMobile.png";
...
function Project({ item }: TprojectReceived) {
  ...
  const ThemeForComponent = ThemeIsDark
    ? "text-white bg-gray-800 duration-500  border-gray-700 "
  ...
  function ReturnImage(projectName: string, screenType: string) {
    if (projectName === "FalaTú" && screenType === "desktop") {
      return ImgFalaTuDesktop;
    } else if (projectName === "FalaTú" && screenType === "mobile") {
      return ImgFalaTuMobile;
    }
    if (projectName === "Catálogo de produtos" && screenType === "desktop") {
      return ImgCatalogProductDesktop;
    } else if (
      projectName === "Catálogo de produtos" &&
      screenType === "mobile"
    ) {
      return ImgCatalogProductMobile;
    } else if (
      projectName === "Cadastrador de membros" &&
      screenType === "desktop"
    ) {
        ...
        >
          <div className=" mb-4 flex w-full justify-center max-h-96">
            <div className=" flex max-w-[21rem] w-screen300:mx-1 w-screen400:max-w-[17rem] ">
              <Image
                src={ReturnImage(item.name, "desktop")}
                width={1000}
                height={1000}
                alt="image"
                onClick={() => {
                  setModal({
                    IsOpen: true,
                    img: ReturnImage(item.name, "desktop"),
                  });
                }}
              ...
              <Image
                src={ReturnImage(item.name, "mobile")}
                alt="image"
                width={1000}
                height={1000}
                onClick={() =>
                  setModal({
                    IsOpen: true,
                    img: ReturnImage(item.name, "mobile"),
                  })
                }
...
w-screen400:max-h-[5.7rem] w-screen400:rounded-md w-screen400:border-[3px] w-screen400:top-[3.7rem] `}
 ...
 hover:bg-gray-700 border border-gray-700 w-screen350:max-w-[10rem]`}
            >
              <TbWorldWww className=" mr-[2px] text-xl" />
              Ver site
            </Link>
